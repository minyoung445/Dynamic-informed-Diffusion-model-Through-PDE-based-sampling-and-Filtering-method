...
from src.experiment_types.PDE_interpolation import InterpolationExperiment
    ...
    def sample_loop(
        ...
        for s, s_next, s_nnext in tqdm(
            ...
            if s_next <= self.num_timesteps - 1:
                ...
                # print(f"[DEBUG] x_interpolated_s_next = {x_interpolated_s_next}")  
            else:
...
class DYffusion(BaseDYffusion):
    ...
    def __init__(
        ...
        ukf_latent_dim: int = 10,
        process_cov: float = 1e-2,
        meas_cov: float = 1e-1,
        *args,
        **kwargs,
    ):
        ...
        self.ukf_latent_dim = ukf_latent_dim
        self.ukf_process_cov = process_cov
        self.ukf_meas_cov = meas_cov
        self.save_hyperparameters(ignore=["interpolator", "model"])
        ...
        # UKF sigma points
        self.ukf_points = MerweScaledSigmaPoints(n=self.ukf_latent_dim, alpha=0.1, beta=2.0, kappa=0.0)

        # Embed & projection networks
        self.ukf_embed = None
        self.ukf_proj = None        
    ...
    def _interpolate(
        ...
        **kwargs
        ...
        # print("[DEBUG] inside _interpolate")
        # print("    initial_condition:", initial_condition.shape)
        # print("    x_last:", x_last.shape)
        # print("    t:", t)
        # print("    static_condition:", static_condition.shape if static_condition is not None else None)
        ...
        if isinstance(interpolator_outputs, dict):
            if "preds" in interpolator_outputs:
                interpolator_outputs = interpolator_outputs["preds"]
            else:
                raise KeyError(f"[ERROR] 'preds' not in interpolator_outputs: {interpolator_outputs.keys()}")
        elif isinstance(interpolator_outputs, torch.Tensor):
            pass  # use as-is
        else:
            raise TypeError(f"[ERROR] interpolator.predict() returned unsupported type: {type(interpolator_outputs)}")

        # print("    interpolator_outputs (final shape):", interpolator_outputs.shape)
    ...
    def _ukf_fx_numpy(
        ...
        dt: float,
        x_end: torch.Tensor,
        forward_cond: torch.Tensor,
        static_cond: Optional[torch.Tensor],
        i_n: float,
        ...
        flat = self.ukf_proj(z)
        xt = flat.view(batch_shape)  # e.g., (B, H, W) or (B, C, H, W)
        if xt.dim() == 3:
            xt = xt.unsqueeze(1)  # (B, 1, H, W)

      
        assert x_end.shape[-2:] == xt.shape[-2:], f"Shape mismatch: x_end {x_end.shape}, xt {xt.shape}"

       
        t_val = max(i_n, 1e-3)  # float
        t_interp = torch.tensor([t_val], device=device)

        
        ipred = self._interpolate(
    ...
    def _ukf_hx(self, x, *args, **kwargs):
        device = next(self.model.parameters()).device
        return torch.from_numpy(x).float().to(device).detach().cpu().numpy()


    def fx_numpy_batch(self, sigmas: np.ndarray, t_val: float, x_end: torch.Tensor,
                    forward_cond: torch.Tensor, static_cond: Optional[torch.Tensor], shape_b: torch.Size) -> np.ndarray:
        """Batch version of fx_numpy for sigma point propagation."""
        device = next(self.model.parameters()).device
        sigmas_tensor = torch.tensor(sigmas, dtype=torch.float32, device=device)
        flat = self.ukf_proj(sigmas_tensor)
        xt = flat.view(-1, *shape_b)
        if xt.dim() == 3:
            xt = xt.unsqueeze(1)

        t_interp = torch.full((xt.shape[0],), t_val, dtype=torch.float32, device=device)

        with torch.no_grad():
            ipred = self._interpolate(
                initial_condition=xt,
                x_last=x_end.expand(xt.shape[0], *x_end.shape[1:]),
                t=t_interp,
                static_condition=static_cond.expand(xt.shape[0], *static_cond.shape[1:]) if static_cond is not None else None,
            )
            y = self.model.predict_forward(ipred, time=t_interp, condition=forward_cond.expand_as(ipred))
    ...
    def p_losses(self, xt_last: Tensor, condition: Tensor, t: Tensor, static_condition: Tensor = None) -> dict:
        lam1 = self.hparams.lambda_reconstruction
        lam2 = self.hparams.lambda_reconstruction2
        lam3 = getattr(self.hparams, "lambda_nll", 1.0)

        device = xt_last.device
        B = xt_last.size(0)
        flat_dim = xt_last.view(B, -1).shape[1]
        D_latent = self.ukf_latent_dim

        if self.ukf_embed is None:
            self.ukf_embed = nn.Linear(flat_dim, D_latent).to(device)
            self.ukf_proj = nn.Linear(D_latent, flat_dim).to(device)

      
        if (
            not hasattr(self, "P_latent")
            or self.P_latent is None
            or not hasattr(self, "_P_latent_B_prev")
            or self._P_latent_B_prev != B
            or self.P_latent.shape != (B, D_latent, D_latent)
        ):
            I = torch.eye(D_latent, device=device).unsqueeze(0).expand(B, -1, -1)
            self.P_latent = I * self.ukf_process_cov
            self._P_latent_B_prev = B  
        ...
        if t_nonzero.any():
            with torch.no_grad():
                x_interpolated = self.q_sample(
                    ...
                    t=t[t_nonzero],
                    static_condition=static_condition[t_nonzero] if static_condition is not None else None,
                    num_predictions=1,
                )
        ...
        z_pred = self.ukf_embed(xt_last_pred.view(B, -1))
        z_obs = self.ukf_embed(xt_last.view(B, -1))

        points = self.ukf_points
        z_post_list, P_post_list = [], []

        for b in range(B):
            x = z_pred[b].detach().cpu().numpy()
            P = self.P_latent[b].detach().cpu().numpy()
            sigmas = points.sigma_points(x, P)
            Wm, Wc = points.Wm, points.Wc

            x_end_b = condition[b:b+1]
            fcond_b = condition[b:b+1]
            scond_b = static_condition[b:b+1] if static_condition is not None else None
            shape_b = xt_last[b].shape
            t_val = max(float(t[b].item()) / self.num_timesteps, 1e-3)

            sigmas_f = self.fx_numpy_batch(sigmas, t_val, x_end_b, fcond_b, scond_b, shape_b)
            x_pred = np.sum(Wm[:, None] * sigmas_f, axis=0)
            P_pred = sum(Wc[i] * np.outer(sigmas_f[i] - x_pred, sigmas_f[i] - x_pred) for i in range(sigmas_f.shape[0]))
            P_pred += np.eye(D_latent) * 1e-5

            y = z_obs[b].detach().cpu().numpy() - x_pred
            S = P_pred + np.eye(D_latent) * self.ukf_meas_cov
            K = np.linalg.solve(S.T, P_pred.T).T
            x_post = x_pred + K @ y
            P_post = P_pred - K @ S @ K.T

            z_post_list.append(torch.tensor(x_post, device=device, dtype=torch.float32))
            P_post_list.append(torch.tensor(P_post, device=device, dtype=torch.float32))

        z_post = torch.stack(z_post_list, dim=0)
        P_post = torch.stack(P_post_list, dim=0)
        self.P_latent = P_post.detach()

      
        flat_corr = self.ukf_proj(z_post).view_as(xt_last_pred)
        loss_mse = self.criterion(flat_corr, xt_last)

      
        diag_latent_cov = torch.diagonal(P_post, dim1=1, dim2=2) + 1e-6
        diag_flat_cov = diag_latent_cov.mean(dim=1, keepdim=True).expand(-1, flat_dim)
        std_flat = diag_flat_cov.sqrt()
        dist = torch.distributions.Independent(
            torch.distributions.Normal(flat_corr.view(B, -1), std_flat), 1
        )
        loss_nll = -dist.log_prob(xt_last.view(B, -1)).mean()

     
        tnot_last = t <= self.num_timesteps - 2
        if lam2 > 0 and tnot_last.any():
            t2 = t[tnot_last] + 1
            cond_nl = condition[tnot_last]
            sc_nl = static_condition[tnot_last] if static_condition is not None else None
            x0_nl = flat_corr[tnot_last].view(-1, *xt_last.shape[1:])
            with torch.no_grad():
                xip2 = self.q_sample(x_end=cond_nl, x0=x0_nl, t=t2, static_condition=sc_nl, num_predictions=1)
            xlp2 = self.predict_x_last(condition=cond_nl, x_t=xip2, t=t2, static_condition=sc_nl)
            loss_forward2 = self.criterion(xlp2, xt_last[tnot_last])
        else:
            loss_forward2 = torch.tensor(0.0, device=device)

        total_loss = lam1 * loss_mse + 0.00001 * loss_nll + 0.5 * loss_forward2
        log_prefix = "train" if self.training else "val"
        return {
            "loss": total_loss,
            f"{log_prefix}/loss_mse": loss_mse,
            f"{log_prefix}/loss_nll": loss_nll,
