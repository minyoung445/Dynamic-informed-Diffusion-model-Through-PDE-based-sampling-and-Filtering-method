...
class UNet(BaseModel):
    ...
    #     print(f"[DEBUG] num_conditional_channels: {self.num_conditional_channels}")
    ...
    def forward(self, inputs, time=None, condition=None, return_time_emb: bool = False, **kwargs):
        ...
        # Safety check: inputs already include condition?
        expected_total_channels = self.num_input_channels + self.num_conditional_channels
        if inputs.shape[1] == expected_total_channels:
            x = inputs
        else:
            if self.num_conditional_channels > 0:
                assert condition is not None, (
                    f"Expected condition input with {self.num_conditional_channels} channels, but got None.\n"
                    f"inputs.shape = {inputs.shape}"
                )
                x = torch.cat([inputs, condition], dim=1)
            else:
                x = inputs
                assert condition is None, "Model not expecting condition input, but got one."
