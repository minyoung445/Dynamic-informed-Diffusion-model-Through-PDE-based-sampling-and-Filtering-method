...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            time_emb_proj = self.mlp(time_emb)
            time_emb_proj = rearrange(time_emb_proj, "b c -> b c 1 1")
            scale_shift = time_emb_proj.chunk(2, dim=1)
...
class Unet(BaseModel):
    ...
    def forward(self, x, time=None, condition=None, return_time_emb: bool = False):
        # ---------------------------------------------------------------------
        # 0) 입력 텐서를 UNet 레이어 수만큼의 2^n 배수로 맞춰 패딩
        # ---------------------------------------------------------------------
        batch, _, orig_h, orig_w = x.shape
        factor = 2 ** len(self.downs)  # downs 단계 갯수에 따른 배수 계산 (예: 3단계 → 8)
        pad_h = (factor - (orig_h % factor)) % factor
        pad_w = (factor - (orig_w % factor)) % factor

        if pad_h > 0 or pad_w > 0:
            # (left, right, top, bottom) 순서로 패딩. 왼쪽/위는 0, 오른쪽/아래만 pad
        ...
        # # 디버그 출력 (필요시 제거 가능)
        # print("=" * 50)
        # print(f"[DEBUG][Unet.forward] input x.shape (padded): {x.shape}")
        # if condition is not None:
        #     print(f"[DEBUG][Unet.forward] condition.shape (padded): {condition.shape}")
        # if time is not None:
        #     print(f"[DEBUG][Unet.forward] time.shape: {time.shape}")
        # print("=" * 50)

        # ---------------------------------------------------------------------
        # 1) 조건 채널이 존재할 때만 concat
        # ---------------------------------------------------------------------
        if self.num_conditional_channels > 0:
            # condition이 None이면 assertion 발생
            assert condition is not None, "condition is None but num_conditional_channels > 0"
            x = torch.cat((condition, x), dim=1)
        else:
            # 조건 채널이 없다면 condition 역시 None이어야 함
            assert condition is None, "condition should be None when num_conditional_channels=0"

        # ---------------------------------------------------------------------
        # 2) UNet 본체 연산
        # ---------------------------------------------------------------------
        orig_x_shape = x.shape[-2:]  # 패딩 후 크기 (예: (224, 48))
        x = self.upsampler(x) if exists(self.upsampler) else x
        x = self.init_conv(x)
        r = x.clone() if self.hparams.input_dropout <= 0 else self.dropout_input_for_residual(x)
        ...
        # ---------------------------------------------------------------------
        # 4) 업샘플러가 있으면 orig_x_shape(패딩 후 크기)로 interpolate,
        #    이후 맨 처음에 pad했던 부분을 잘라내어 원본 크기 (orig_h, orig_w)로
        # ---------------------------------------------------------------------
