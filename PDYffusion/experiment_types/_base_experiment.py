...
class BaseExperiment(LightningModule):
    ...
    def predict(
        self, inputs: Tensor, num_predictions: Optional[Union[int, Tensor]] = None,
        reshape_ensemble_dim: bool = True, **kwargs
    ) -> Dict[str, Tensor]:
        base_num_predictions = self.hparams.num_predictions
        # print(f"[DEBUG] num_predictions in predict: {num_predictions} (shape: {getattr(num_predictions, 'shape', 'not tensor')})")
        if num_predictions is not None:
            if isinstance(num_predictions, torch.Tensor):
                if num_predictions.numel() == 1:
                    num_predictions = num_predictions.item()  # 단일 원소 텐서일 경우만
                else:
                    raise ValueError(f"`num_predictions` must be scalar or single-element tensor, but got shape {num_predictions.shape}")
            self.hparams.num_predictions = num_predictions
        else:
