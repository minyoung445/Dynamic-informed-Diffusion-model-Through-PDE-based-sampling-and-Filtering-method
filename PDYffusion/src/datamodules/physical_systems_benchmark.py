...
class PhysicalSystemsBenchmarkDataModule(BaseDataModule):
    def __init__(
        ...
        **kwargs,
        ...
        raise_error_if_invalid_value(
            physical_system, possible_values=["navier-stokes", "spring-mesh","wave"], name="physical_system"
        )
        ...
        if physical_system == "navier-stokes":
            ...
            self.subdirs["predict"] = self.subdirs["val"]
        elif physical_system == "wave":
            _first_subdir = "wave"
            self.subdirs = {
                "train": "wave-runs_train-wave-n50-t10205-n0_00001",
                "val": "wave-runs_val-wave-n3-t10205-n0_00001",
                "test": f"wave-runs_eval-wave-n6-t10204-n0_tagcors1_00001-{ood_infix}wave-runs_eval-outdist-wave-n6-t10204-n0_tagcors1_00001",
            }
            self.subdirs["predict"] = self.subdirs["val"]            
        else:
        ...
        #     join(self.hparams.data_dir)
        # ), f"Could not find data directory {self.hparams.data_dir}. Did you download the data?"
        # log.info(f"Using data directory: {self.hparams.data_dir}")
        assert os.path.isdir(self.hparams.data_dir), f"Could not find data directory {self.hparams.data_dir}. Please specify the data directory using the ``datamodule.data_dir`` option."
        log.info(f"Using data directory: {self.hparams.data_dir}")
    ...
    def _get_split_dir(self, split: str) -> str | None:
        if self.subdirs[split] in ["", None]:
            ...
            return join(self.hparams.data_dir, self.subdirs[split])

    def _get_numpy_filename(self, split: str) -> Optional[str]:
        return join(self.hparams.data_dir, self.subdirs[split], "trajectories.npz")
    ...
    def create_dataset_multi_horizon(
        ...
        for i in range(n_trajectories):
            ...
            elif self.hparams.physical_system == "wave"   :
                traj_i.trajectory_meta["features"] = traj_i.features
                traj_i.trajectory_meta.setdefault("state_names", ["q", "p"])
    ...
    def boundary_conditions(
        ...
        if self.hparams.physical_system == "navier-stokes":
            ...
            bc = getattr(self.hparams, "wave_boundary", "none")

            if bc == "none":
                # wave 데이터는 보통 별도의 고정 마스크/지오메트리가 없으므로 그대로 반환
                pass

            else:
                # preds shape: (..., C, H, W) 를 가정 (C=2: q,p, H=grid, W=1)
                # spring-mesh 분기와 동일하게 5D(ensemble)도 대응
                def _apply_to(tensor: Tensor, b_index: int):
                    # tensor[..., C, H, W]
                    if bc == "dirichlet0":
                        # 양 끝 H=0, H=H-1에서 (q,p)=0
                        tensor[..., :, 0, :]  = 0
                        tensor[..., :, -1, :] = 0
                    elif bc == "periodic":
                        # 좌우 주기 경계: 양 끝을 이웃값으로 맞춤
                        # 왼쪽 끝(0) <- 1, 오른쪽 끝(-1) <- -2
                        tensor[..., :, 0,  :] = tensor[..., :, 1,  :].clone()
                        tensor[..., :, -1, :] = tensor[..., :, -2, :].clone()
                    else:
                        raise ValueError(f"Unknown wave_boundary={bc}")
                    return tensor

                for b_i in range(batch_size):
                    if preds.ndim == 5 and preds.shape[1] == batch_size:
                        # (T, B, C, H, W) 같은 형태를 가정한 케이스
                        preds[:, b_i, ...] = _apply_to(preds[:, b_i, ...], b_i)
                    else:
                        preds[b_i, ...] = _apply_to(preds[b_i, ...], b_i)
